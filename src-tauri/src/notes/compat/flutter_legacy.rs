...
pub use legacy_paths::existing_legacy_notes_files;
...
fn stable_legacy_id_seed(
    ...
    let x = pick_f64(obj, &["x"])
        .map(|v| v.to_string())
        .unwrap_or_default();
    let y = pick_f64(obj, &["y"])
        .map(|v| v.to_string())
        .unwrap_or_default();
    let width = pick_f64(obj, &["width"])
        .map(|v| v.to_string())
        .unwrap_or_default();
    let height = pick_f64(obj, &["height"])
        .map(|v| v.to_string())
        .unwrap_or_default();
...
fn note_from_value(value: &Value) -> Option<Note> {
    ...
    let updated_at = updated_at_raw.clone().unwrap_or_else(|| created_at.clone());

    Some(Note {
        id: pick_string(obj, &["id"]).unwrap_or_else(|| {
            let seed =
                stable_legacy_id_seed(obj, created_at_raw.as_deref(), updated_at_raw.as_deref());
