...
- 第三小步：抽出 task draft facade：已完成第一步。
  - 候选：`workspace/pomodoro/focus-task-draft-controller.js`
  - 已覆盖：start/select/add/update/remove、toggle weekday、open/toggle settings、save config。
...
- 抽出 route workspace bridge：已完成第一步。
  - 候选：`src/lib/workspace/controllers/workspace-route-workspace-bridge.js`
  - 已覆盖：focus/settings/window controller config bridge。
- 保留 route 入口作为：
  - 状态初始化；
  - controller 组装；
  - 顶层布局组件绑定。
