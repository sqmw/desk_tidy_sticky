...
E-004 补充：Probe提交`3e85be7`已补齐Rust真实进程重启的数据保留、旧会话重放拒绝与存活JS旧连接拒绝；证据在同级Probe的`docs/persistence-evidence.md`，合同在`docs/persistence.md`。上述“重启数据未验证”缺口由本次证据补齐，其余容器/内存/移动限制仍有效。S2按最小验证闭环完成，S3进入前置准备；无产品源码Promotion，不代表安装管理或产品持久化迁移已完成。

S3 当前顺序：确认Android可运行验证载体与设备，选择移动端可用的隔离容器并接同一合同；准备iOS模拟器/target及签名渠道；两端分别验证外部脚本、文件导入、后台/锁屏提醒与撤销。macOS子进程方式不能直接作为移动实现；内存硬上界、JS/Web UI兼容和引擎风险纳入S3平台验证及S4路线裁决，不放宽最终Task验收。当前仅完成只读环境复核，未下载大型工具链、未运行手机端。
...
E-006续接：用户完成系统选文件后，Probe证据提交`e79ec60`已补齐iOS真实导入/持久化、进程重开、后台锁屏通知及取消；最新构建也已部署并复核。此前文件选择阻塞解除，最小双模拟器课程链路已有各端证据，未进行产品Promotion。真机/渠道、撤权与重启矩阵、JS资源限制和完整安全边界仍不由本结果推出；阶段状态只见TODO/STEPS。
