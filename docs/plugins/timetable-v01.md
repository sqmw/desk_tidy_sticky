...
版本推进：真实demo、schema v2与插件0.2.0的当前合同见[schema v2](timetable-schema-v2.md)及D-EXT-008。本文件保留最初v0.1接入和历史证据；v1读取与固定旧包继续兼容，不表示当前只能导入v1。
...
实施阶段：E-003已接入工作台主内容区，当前源码导航与证据见[实施记录](workspace-integration-implementation.md)。前段“只整理方案”是E-002阶段记录；旧独立路由只保留兼容，工作台默认入口不再新建窗口。
