...
- Development builds omit the autostart plugin and disable its settings controls based on backend capability. Existing machine login entries are unchanged. Windows monitor geometry uses a single window-scale coordinate space; macOS retains its platform-specific point conversion.
