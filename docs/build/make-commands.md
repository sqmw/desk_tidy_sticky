...
`pnpm test:browser` 执行 `scripts/tests/review-editor-dom.mjs`：在系统临时目录建立夹具，复用实际 Svelte 编辑器组件，以模拟存储验证失焦/关闭、保存失败/重试、外部冲突、Enter 分块成功与回滚。只监听 loopback 随机端口；关闭自己创建的浏览器和服务，保留截图/夹具路径便于诊断，不读写应用数据。
