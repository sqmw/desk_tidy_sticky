...
1. 保留原产品策略：进入控制 / 编辑态时向下扩高窗口，正文区域尺寸和坐标不变；工具操作全部直达，不使用“更多”菜单。
2. 工具栏保留新的紧凑视觉，在宽窗口单行展示；窄窗口自然向下换行，并按实际高度扩窗。
3. 移除顶部拖拽柄、活动块整框、额外块间距和列表二次缩进；非活动内容不因进入编辑态发生位置变化。
...
纠偏证据：组件预览测得 ordered-list 编辑前后 `top/left` 相同，高度仅有 `0.09px` 浏览器舍入差；h1 编辑后后续内容位移为 `0.20px` 舍入差。288px 窄窗下 10 个按钮全部可见，工具栏向下换为两行。真实 Tauri 窗口仍需人工回归原生扩窗、中文输入法、窗口拖动、图片粘贴与外部冲突流程。
