...
当前代码仍为Mac独立窗口首版，实测见[首版记录](plugins/mac-product-flow.md)。用户要求按调研整理[主窗口接入方案](plugins/workspace-integration-plan.md)，本轮仅文档，实施尚未开始；阶段及证据见[执行记录](STEPS.md#t-plugin-host)。Task整体仍保留提醒恢复、第三方安全及后续平台验收；当前不推进移动适配。
