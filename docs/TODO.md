...
纠偏证据：组件预览测得 ordered-list 编辑前后 `top/left` 相同，高度仅有 `0.09px` 浏览器舍入差；h1 编辑后后续内容位移为 `0.20px` 舍入差。第二轮修复移除了误伤整块正文的拖动拦截，并用可测试窗口框架保持贴纸矩形坐标。第三轮把宽窗顶部/底部控制轨收口到约 233px/343px；300px 窄贴纸顶部保持 40px，底部两行降至 79px，10 个按钮全部可见，正文矩形仍为 300×500。真实 Tauri 窗口仍需人工回归原生双向扩窗、中文输入法、窗口拖动、图片粘贴与外部冲突流程。
