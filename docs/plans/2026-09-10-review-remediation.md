...
以上系统操作限制为 E-001 的执行边界。2026-09-10 用户追加“直接继续推进，给你所有权限”，E-002 获准备份/更新本机正式应用、管理相关实例和自启登记并继续 S5 实机验证；仍不对外发布、不迁移数据、不破坏历史证据。先核对实际路径并备份，构建/签名检查后替换，最后验证 launchd 启动和菜单栏交互；真实重启与物理多屏的证据不由模拟环境替代。
...
1.2.6 已构建、签名核验并替换本机安装版；旧应用/数据/登录项完整备份。候选原生新建→编辑→关闭验证通过，样例已归档，26 条原笔记正文与业务状态不变。现有正确路径 LaunchAgent 初次拒绝新包签名，重新加载登记后稳定 running；原生 About 确认 1.2.6。详细工件、哈希与证据边界见 [本机安装验收记录](../releases/2026-09-10-local-1.2.6-acceptance.md)。

托盘直接点击结果已向用户请求；当前工具不能定位系统状态栏区域。Windows WMI 仅一块活动物理屏幕，不能执行混合 DPI 双屏现场验收。真实重启也未执行，不把 launchd bootstrap 记为整机重启。
