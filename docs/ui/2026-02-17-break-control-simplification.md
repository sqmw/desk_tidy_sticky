...
文件：
- `src/lib/components/workspace/WorkspaceFocusHub.svelte`
- `src/lib/workspace/break-control/break-control-controller.js`
- `src-tauri/src/breaks/reminder.rs`

问题：
1. 当短休和长休在同一秒同时到点时，前后端此前各自判定，容易出现长短休状态抖动。
2. 用户期望是：冲突时长休优先，长休结束后短休重新计时，而不是立刻补发短休。

调整：
1. 抽出统一的到点选择规则，冲突时始终优先长休。
2. 前端 tick、进度展示和 Rust watchdog 统一采用同一优先级。
3. 长休开始后，短休的下一次触发点仍按新的专注起点重算，不保留旧的“已到点”状态。
4. 2026-07-02 补充修正：到点判定与“下一个休息类型”分离；不到点时到点判定必须返回空，避免跳过或自动结束后下一秒又重新触发休息。

结果：
1. 冲突秒不会再在短休和长休之间来回切换。
2. 长休结束后，短休从新的计时点重新开始，不会立刻补触发。
