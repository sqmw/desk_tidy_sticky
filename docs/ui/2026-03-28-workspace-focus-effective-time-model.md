...
## 2026-03-30 补充：任务改为可重复轮次模型

### 判定
- 类型：`设计调整`

### 问题
1. 原先任务区与统计区仍保留“达标一次即完成”的单次语义。
2. 这与真实使用不一致，因为同一任务在一天内可能被反复执行多轮。
3. 小数番茄作为主计数也容易把“任务次数”和“番茄次数”混在一起。
...
### 结果
1. 一个任务可以自然地完成多次，不再受单次达标语义限制。
2. `任务次数` 与 `🍅次数` 各自表达清晰，避免再出现 `0.8` 这种含义模糊的主计数。

## 2026-03-30 补充：番茄设置面板删除休息控制残留项
...
1. `番茄钟` 的设置面板里混入了：
   ...
   - 长休时长
   - 延后次数 / 严格休息模式
2. 这些配置属于 `休息控制`，不属于番茄任务本身，容易让用户误以为两者强耦合。
...
1. `番茄钟 -> 编辑设置` 现在只保留：
   ...
   - `任务开始提醒`
   - `提前提醒（分钟）`
2. 所有休息节奏与休息策略配置继续保留在 `休息控制` 模块。
3. 面板底部增加提示文案，明确这块只影响番茄任务。

### 代码位置
- `/Users/sunqin/study/language/rust/code/desk_tidy_sticky/src/lib/components/workspace/focus/WorkspaceFocusSettingsPanel.svelte`
- `/Users/sunqin/study/language/rust/code/desk_tidy_sticky/src/lib/components/workspace/WorkspaceFocusHub.svelte`
