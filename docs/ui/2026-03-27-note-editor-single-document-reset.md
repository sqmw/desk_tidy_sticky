...
### 2026-04-15 补充：标签条继续压缩
- 文件：
  - `src/lib/components/note/NoteTagBar.svelte`
  - `src/lib/components/note/NoteTagsEditor.svelte`
- 调整：
  - 收紧标签条 padding、gap 和标题权重，避免头部占用过高。
  - 移除左侧固定“标签”标题，释放更多横向空间给 chip 与 `+标签` 追加入口。
  - 查看态隐藏 `+标签` 追加入口，仅在编辑态展示，进一步降低顶部占用。
  - 查看态下若无任何标签（含 Q1~Q4），则整个标签条不渲染，避免顶部留白。
  - 标签 chip 改为更紧凑尺寸，输入框改成末尾追加式 `+标签`，不再以独立大输入框默认占据下一行。
  - 输入聚焦后再扩展宽度，既保留联想输入，又避免正文被头部挤压。
