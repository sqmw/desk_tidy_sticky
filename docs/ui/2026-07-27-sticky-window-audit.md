...
- **S5 工具栏与控制态重构（已按实测纠偏）**：B1 全部操作直接展示，不引入溢出菜单；B2 popover 锚定重写（复用 workspace 成熟实现）；B5 进入控制态时向下扩高窗口，保持正文区域尺寸和坐标；B7 Esc 分层；B10 标签常显策略。
...
- B1 / B2 / B5：最终保留向下扩高窗口的原产品策略；全部操作直接展示，不使用更多菜单。工具栏保留紧凑视觉，宽窗单行、窄窗向下换行，扩窗高度继续按实际工具栏高度计算。
- B3 / B4 / B6 / B7：`data-no-drag` 生效，正文可选择复制；移除新增顶部拖拽柄，保留空白区域原拖拽逻辑；滚动条恢复为细轨；Esc 先关闭浮层。
