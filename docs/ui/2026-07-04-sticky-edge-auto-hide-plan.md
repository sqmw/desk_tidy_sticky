...
1. 仅当贴纸已经钉在桌面、当前处于置顶显示、且单张贴纸开启自动隐藏时，拖到屏幕任一边缘后才可自动收起，只保留一条可见边。
...
自动隐藏的运行时前置条件不建议只靠 `autoHideEnabled` 表达。实现时应额外检查：

1. note 当前已 pinned，且对应 `note-*` 贴纸窗口存在。
2. 当前显示模式是置顶显示。
3. 当前不在编辑态，也没有正在拖动 / resize。
4. 当前窗口所在 monitor 可解析，或能够安全回退到 primary / virtual screen。

任一条件不满足时，`autoHideEnabled` 可以保留为用户偏好，但不得实际执行自动收起。
...
| 状态 | 进入条件 | 允许动作 |
|---|---|---|
| `visible_unarmed` | 未启用、不靠边、未钉在桌面或非置顶显示 | 正常拖动 / 编辑 |
| `docked_visible` | 已钉在桌面、置顶显示、启用自动隐藏且拖到边缘 | 延迟收起 / 取消启用 / 手动拖离 |
| `hidden` | 已移动到隐藏坐标 | 快捷键显示 / 按钮显示 / 取消启用 |
| `revealed` | 从隐藏状态临时显示 | 再次快捷键收起 / 用户拖动则变为 `visible_unarmed` |

关键规则：

1. 自动收起只允许作用于“已钉在桌面 + 置顶显示 + autoHideEnabled”的贴纸。
2. 用户主动拖动贴纸时，如果当前是 `hidden`，先切回完整显示位置再开始拖动。
3. 拖动结束后才计算自动隐藏，拖动过程中不写隐藏坐标。
4. 贴纸进入编辑态时不自动收起，避免正在输入时窗口消失。
5. 用户拖离边缘超过阈值后，清空 `autoHideEdge`，状态回到 `visible_unarmed`。
6. 取消钉住、归档、删除、关闭窗口时，不额外执行自动隐藏，只走现有窗口同步。
...
- `hide_note_to_edge(id)`：读取 note、置顶 / 钉住状态和窗口尺寸；只有满足前置条件时才计算隐藏坐标并移动窗口。
- `reveal_note_from_edge(id)`：移动到 `autoHideVisibleX/Y`，必要时 `show` 且不激活。
- `toggle_hidden_stickies()`：全局快捷键入口；若存在 hidden 贴纸则全部显示，否则只把已钉在桌面、置顶显示且已 docked 的贴纸收起。
...
- 自动隐藏开关只在贴纸已经钉在桌面且处于置顶显示时可用；其他状态可禁用或隐藏，避免用户误以为普通窗口也会自动贴边隐藏。
...
推荐采用“每张贴纸持久化自动隐藏状态 + 统一 Rust 命令移动窗口 + 前端只负责拖动后判定和按钮状态”的方案，但实际自动收起必须受“已钉在桌面 + 置顶显示”约束。
...
1. 单张贴纸在已钉在桌面且置顶显示时开启自动隐藏，拖到左 / 右 / 上 / 下边缘后只留下 `hiddenSliverPx` 可见边。
2. 快捷键能显示所有隐藏贴纸，再次触发能收起已 docked 贴纸。
3. 面板 / workstation 的按钮能显示指定隐藏贴纸。
4. 编辑态不自动隐藏。
5. 未钉在桌面、非置顶显示或普通面板窗口不会触发自动隐藏。
6. 重启后隐藏贴纸可通过快捷键 / 面板按钮恢复。
7. macOS 第一次隐藏和第一次显示都一次生效。
8. `make check`、edge 计算 smoke、Windows/macOS 手动回归通过。
