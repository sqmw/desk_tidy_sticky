# Workspace 主题系统升级：内置主题 + 自定义主题

## 背景
- 之前工作台仅支持浅色/深色二选一，无法满足用户对品牌感和个性化配色的需求。
- 目标是提供两层能力：
  - 开箱即用：内置多个可选主题。
  - 深度定制：用户可直接编写主题 CSS 覆盖样式。
...
- 设置面板新增“自定义主题”编辑区，输入后即时应用，并自动保存（防抖写入）。
- 清空按钮一键恢复默认样式。

### 3) 页面渲染策略
- 主题预设通过根容器内联变量应用（`style` 注入 CSS variables）。
- 用户自定义主题 CSS 通过 `<svelte:head><style>...</style></svelte:head>` 注入。
...
- 设置面板：
  - 主题预设与自定义主题在同一块区域，避免功能割裂。
  - 自定义主题：按需覆盖任何选择器（建议以 `.workspace` 作用域起手）。
  - 新增“复制完整模板”：一键复制完整主题变量、模块默认样式与全组件选择器索引。
...
## 说明
- 本次先落地“预设 + 自定义主题”能力，不涉及“在线主题市场”。
- 后续可在当前结构上继续加：
  - 主题预览缩略图
  - 主题包（JSON）在独立入口的导入导出（不与自定义 CSS 混排）
  - 自定义 CSS 语法校验/格式化
