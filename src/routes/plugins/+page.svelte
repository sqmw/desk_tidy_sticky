<script lang="ts">
  ...
  async function task(work:()=>Promise<void>){if(busy)return;busy=true;error='';try{await work();}catch(e){error=String(e instanceof Error?e.message:e);}finally{busy=false;}}
  ...
  async function validate(){if(!runtime)throw new Error('请先打开插件');draft=null;const value=await runtime.call('parse',form);await showRows(value);draft=value;validated=form;}
