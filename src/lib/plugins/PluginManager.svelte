<script lang="ts">
  ...
  let { snapshot, changed, open, activity = () => {} }: { snapshot: PluginSnapshot; changed: (value: PluginSnapshot) => Promise<void>; open: () => void; activity?: (busy: boolean) => void } = $props();
  let busy = $state(false), error = $state(''), notice = $state(''), raw = $state('');
  let candidate = $state<any>(null), consent = $state(false), confirm = $state('');
  $effect(() => { activity(busy); });
...
<section>
  ...
  {#if snapshot.manifest}
    ...
    {#if confirm}<PluginDialog title={confirm === 'uninstall' ? '卸载课表插件？' : '停用课表插件？'} {busy} cancel={() => confirm = ''}>{#if error}<p class="error" role="alert">{error}</p>{/if}<p>未来提醒将被取消，已保存的课表数据会保留。{confirm === 'uninstall' ? '重新安装后可以恢复使用。' : '启用后可以恢复使用。'}</p><div class="actions"><button disabled={busy} onclick={() => lifecycle(confirm)}>确认{confirm === 'uninstall' ? '卸载' : '停用'}</button><button disabled={busy} onclick={() => confirm = ''}>取消</button></div></PluginDialog>{/if}
...
<section><h2>从本地安装</h2><p class="muted">选择 .dtplugin 文件。当前版本仅开放经过审查的官方课表包，其他包和被修改的包会被拒绝。</p>
  ...
  {#if candidate}<PluginDialog title={`安装 ${candidate.name} · ${candidate.version}`} {busy} cancel={() => { candidate = null; raw = ''; consent = false; }}>{#if error}<p class="error" role="alert">{error}</p>{/if}<p>申请的权限：</p><ul><li>独立存储：保存本插件的课表，不访问笔记数据。</li><li>系统提醒：安排与取消上课提醒，是否显示仍受系统权限影响。</li></ul><label class="consent"><input type="checkbox" disabled={busy} bind:checked={consent}/> 我同意以上权限</label><div class="actions"><button class="primary" disabled={busy || !consent} onclick={install}>确认安装</button><button disabled={busy} onclick={() => { candidate = null; raw = ''; consent = false; }}>取消安装</button></div></PluginDialog>{/if}
</section>
