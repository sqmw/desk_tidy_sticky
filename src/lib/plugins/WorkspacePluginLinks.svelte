<script lang="ts">
  ...
  let { collapsed = false, mainTab = '', onNavigate }: { collapsed?: boolean; mainTab?: string; onNavigate: (tab: string) => unknown } = $props();
  ...
  async function open(view: string) { error = ''; try { await onNavigate(view); } catch (e) { error = '无法打开插件：' + String(e); } }
</script>
<div class="links" class:collapsed>
  <button class:active={mainTab === 'plugins'} aria-current={mainTab === 'plugins' ? 'page' : undefined} onpointerdown={event => event.preventDefault()} title="插件" aria-label="插件" onclick={() => open('plugins')}><span aria-hidden="true">◇</span>{#if !collapsed}<span>插件</span>{/if}</button>
  {#if available}<button class:active={mainTab === 'timetable'} aria-current={mainTab === 'timetable' ? 'page' : undefined} onpointerdown={event => event.preventDefault()} title="课表" aria-label="课表" onclick={() => open('timetable')}><span aria-hidden="true">▦</span>{#if !collapsed}<span>课表</span>{/if}</button>{/if}
  {#if error}<p role="alert">{error}</p>{/if}
</div>
<style>
  button.active{background:color-mix(in srgb,var(--ws-accent,#245bd5) 11%,transparent);color:var(--ws-accent,#245bd5);font-weight:700}
